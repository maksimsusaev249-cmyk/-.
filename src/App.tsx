...
// --- INTERACTION STATES ---
  ...
  const [telegramNotificationsEnabled, setTelegramNotificationsEnabled] = useState<boolean>(() => {
    const saved = safeGetItem("gameTelegramNotifications");
    return saved ? JSON.parse(saved) : true;
  });
  ...
  useEffect(() => {
    safeSetItem("gameTelegramNotifications", JSON.stringify(telegramNotificationsEnabled));
  ...
  const [transferPlayerIdInput, setTransferPlayerIdInput] = useState("");

  const switchPlayerProfile = (newId: string) => {
    if (!newId || newId === effectivePlayerId) return;
    safeSetItem("myPlayerIdV9", newId);
    setTransferPlayerIdInput("");
    addToast("🔄 Профиль изменен! Перезагрузка...");
    setTimeout(() => {
                  ...
                  <div className="flex flex-col gap-2 pt-1 border-t border-white/5">
                    ...
                    <div className="flex items-center justify-between mt-1 bg-slate-950 p-2 rounded-xl border border-slate-900">
                      ...
                      <button
                        ...
                        className={`w-8 h-8 flex items-center justify-center rounded-lg transition-all cursor-pointer border-none outline-none ${
                          newClanVoiceEnabled
                            ? "bg-[#e67e22]/20 text-[#e67e22]"
                            : "bg-slate-800 text-gray-500"
                        }`}
                        title={newClanVoiceEnabled ? "Голосовые сообщения включены" : "Голосовые сообщения отключены"}
                      >
                        {newClanVoiceEnabled ? <Mic size={14} /> : <MicOff size={14} />}
                      </button>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex items-center justify-between p-3 bg-slate-950/60 rounded-xl border border-white/5 text-xs">
                    <div className="flex flex-col gap-0.5 max-w-[75%]">
                      ...
                      <span className="text-[9px] text-gray-500 leading-normal">
                        {!currentClanInfo || !currentClanInfo.creatorId
                          ? "🛡️ Нажмите на микрофон, чтобы установить статус создателя клана."
                          : isCreator 
                            ? "🛡️ Вы создатель клана. Нажмите на микрофон для вкл/выкл." 
                    ...
                    {isCreator ? (
                      <button
                        ...
                        className={`w-9 h-9 flex items-center justify-center rounded-xl transition-all cursor-pointer border-none outline-none shrink-0 ${
                          isVoiceEnabled
                            ? "bg-[#e67e22]/20 text-[#e67e22]"
                            : "bg-slate-800 text-gray-500 hover:text-gray-400"
                        }`}
                        title={isVoiceEnabled ? "Голосовые включены (нажмите чтобы выключить)" : "Голосовые отключены (нажмите чтобы включить)"}
                      >
                        {isVoiceEnabled ? <Mic size={16} /> : <MicOff size={16} />}
                      </button>
                    ) : (
                      <div className={`w-9 h-9 flex items-center justify-center rounded-xl border shrink-0 ${
                        isVoiceEnabled
